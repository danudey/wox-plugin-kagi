...
ifeq ($(OS),Windows_NT)
	...
	Get-ChildItem -Path '$(DIST_DIR)' -Directory -Recurse | Where-Object { $$_.Name -eq '__pycache__' } | Remove-Item -Recurse -Force
	...
	New-Item -ItemType Directory -Force -Path '$(DIST_DIR)\icons' | Out-Null
	Copy-Item 'icons\*' -Destination '$(DIST_DIR)\icons' -Recurse -Force
else
	rm -rf $(DIST_DIR)
	...
	mkdir -p $(DIST_DIR)/images $(DIST_DIR)/icons
	cp images/* $(DIST_DIR)/images/
	cp icons/* $(DIST_DIR)/icons/
endif

package: check-init build
ifeq ($(OS),Windows_NT)
	if (Test-Path 'wox.plugin.kagi.zip') { Remove-Item -Force 'wox.plugin.kagi.zip' }
	if (Test-Path 'wox.plugin.kagi.wox') { Remove-Item -Force 'wox.plugin.kagi.wox' }
	Compress-Archive -Path '$(DIST_DIR)\*' -DestinationPath 'wox.plugin.kagi.zip'
	Move-Item 'wox.plugin.kagi.zip' 'wox.plugin.kagi.wox'
	if (Test-Path '$(DIST_DIR)') { Remove-Item -Recurse -Force '$(DIST_DIR)' }
else
	cd $(DIST_DIR) && zip -r "../wox.plugin.kagi.wox" .
	rm -rf $(DIST_DIR)
endif
